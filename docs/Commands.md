...
EnergyToday<x\><a class="cmnd" id="energytoday"></a>|Reset Energy Today values<BR>`0` = reset<BR>`<value>` = set value<BR>`<time>` = `0..4294967295` set StartTotalTime time as epoch value (optional 2nd parameter).<BR>Note: Values have to be set in Wh and are displayed in kWh.
EnergyTotal<x\><a class="cmnd" id="energytotal"></a>|Reset Energy Total values<BR>`0` = reset<BR>`<value>` = set value<BR>`<time>` = `0..4294967295` set StartTotalTime time as epoch value (optional 2nd parameter)<BR>The new value represents start of day, and output for total includes the today value.<BR>Note: Values have to be set in Wh and are displayed in kWh.
