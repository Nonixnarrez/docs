...
EnergyYesterday<x\><a class="cmnd" id="energyyesterday"></a>|Reset Energy Yesterday values<BR>`0` = reset<BR>`<value>` = set value<BR>`<time>` = `0..4294967295` set StartTotalTime time as epoch value (optional 2nd parameter).<BR>Note: Values have to be set in Wh and are displayed in kWh.
